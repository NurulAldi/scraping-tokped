...
while True:
    ...
    try:
        ...
        params_sekarang = params_sekarang.replace(f"start={start_sebelumnya}", f"start={start_sekarang}")

        payload_sekarang = json.loads(json.dumps(base_payload))
        ...
        for produk in produk_di_halaman:
            ...
            harga = produk.get('price', {}).get('number')
        ...
        if total_produk_didapat >= total_data_keseluruhan:
            print("Data semua produk sudah diambil.")
            break
        ...
        print("Respons:", json.dumps(data, indent=2))
        break
